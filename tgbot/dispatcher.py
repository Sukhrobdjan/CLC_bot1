...
from telegram import Bot, CallbackQuery, InlineQueryResultArticle, InputTextMessageContent, Update, BotCommand
...
def inlinequery(update: Update, context: CallbackQuery) -> None:
    ...
    results = [
        ...
        input_message_content=InputTextMessageContent(message_text=f"{post.content}\n{post.img}",
                                                      parse_mode=None)) for post in Post.objects.filter(title__icontains = query )]
